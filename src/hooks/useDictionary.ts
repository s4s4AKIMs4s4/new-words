...
export default function useDictionary() {
  ...
  const getWords = async () => {
    ...
    const languageWords = allWords.learnList.filter((wordObject) => {
      return (
        User.destenationLanguage === wordObject.destenationLanguage &&
        User.sourseLanguage === wordObject.sourseLanguage
      )
    })
  ...
  const filterDeleteWords = (wordObject: IDictionaryElement) =>
    wordObject.sourceWords[0] !== sourseLanguageList.current!.languageList[0]
