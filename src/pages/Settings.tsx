...
function Settings({ navigation }: any) {
  ...
  const sourceOptions = useMemo(() => {
    const sourseList: ISelectOption[] = []
  ...
  return (
    <>
      <View style={tw`bg-[#fff] w-full h-full`}>
        ...
        <View style={tw`px-3`}>
          <Text style={tw`text-sm font-light mt-15 mb-2`}>
            Сhoose the language you already know:
          </Text>
          <SelectElement
            placeholder={
              LanguageList.find((ListItem) => ListItem.value === User.sourseLanguage)?.label
            }
          ...
          <SelectElement
            placeholder={
              LanguageList.find((ListItem) => ListItem.value === User.destenationLanguage)?.label
            }
